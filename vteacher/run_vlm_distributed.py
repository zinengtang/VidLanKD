...
#     model.module.load_state_dict(torch.load('snap/vlm/howto100m_bert_base_vokenhinge_v1/checkpoint-epoch0022/pytorch_model.bin'), strict=False)
#     visn_model.load_state_dict(torch.load('snap/vlm/howto100m_bert_base_vokenhinge_v1/checkpoint-epoch0022/visn_model.chkpt'), strict=False)
    model.module.load_state_dict(torch.load('snap/vlm/howto100m_bert_small_vokenhinge_v1/checkpoint-epoch0022/pytorch_model.bin'), strict=True)
    visn_model.load_state_dict(torch.load('snap/vlm/howto100m_bert_small_vokenhinge_v1/checkpoint-epoch0022/visn_model.chkpt'), strict=True)
    
#     model.module.load_state_dict(torch.load('snap/vlm/howto100m_bert_small_caponly/checkpoint-epoch0106/pytorch_model.bin'), strict=False)
...
#             (voken_labels!=0.0)[:,:,-1].to(args.device)
...
#     visn_model.save_pretrained(output_dir)
...
#             attention_mask = None
        ...
        with torch.no_grad():
            if args.voken_hinge_loss:
                voken_masks = (voken_labels!=0.0)[:,:,-1]
#                 print(voken_labels.size(), voken_labels[voken_labels!=0.0].size())
                visn_output = visn_model(voken_labels, voken_masks, token_type_ids, position_ids)
            else:
                visn_output = None
...
#         model.init_voken_feat_emb(voken_feats)
    ...
    if args.voken_hinge_loss:
#         visn_config = VisnBertConfig.from_pretrained(args.config_name, cache_dir=args.cache_dir,)
