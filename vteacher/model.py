...
# from vteacher.transformer import VideoBert, BertVideoEncoder
# from transformers import (
#     BertConfig,
#     BertForMaskedLM,
#     BertLayer
...
#         if config.voken_hinge_loss:
        self.visual_hinge_head = BertVLMHingeHead(config)
#             self.hinge_loss = paired_hinge_rank_loss contrastive_loss
...
#         if self.voken_class_loss:
#             prediction_scores = self.visual_class_head(sequence_output)
#             voken_class += self.token_cls_loss_fct(
...
# #             voken_label_mask = (voken_labels != -100)               # Get a mask of 
#             voken_regression_loss = self.voken_reg_loss_fct(voken_prediction, voken_features).sum(1).mean()  # [b, len, f]
# #             voken_regression_loss = (voken_regression_loss * voken_label_mask.float())
# #             voken_regression_loss = voken_regression_loss.sum(-1).mean()
...
#         print(token_loss, voken_contrastive_loss)
...
#         else:
#             voken_prediction = None
            
#         if masked_lm_labels is not None:
#             prediction_scores = self.cls(sequence_output)

        return None, voken_prediction, sequence_output
...
#         video_extended_attention_mask = self.bert.get_extended_attention_mask(video_mask, video_mask.shape, video_mask.device)
