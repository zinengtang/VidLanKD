...
class CRD(nn.Module):
    ...
    """
    def __init__(self, s_dim, t_dim, feat_dim, nce_t=0.07, nce_m=0.5, n_data=100*10000):
        super(CRD, self).__init__()
        self.embed_s = Embed(s_dim, feat_dim)
        self.embed_t = Embed(t_dim, feat_dim)
        self.contrast = ContrastMemory(feat_dim, n_data, nce_t, nce_m)
    ...
    def forward(self, f_s, f_t, idx):
        ...
        out_s, out_t = self.contrast(f_s, f_t, torch.tensor(idx).to(f_s.device), None)
#         print(f_s.size(), out_s.size())
